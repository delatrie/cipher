...
{
    [Serializable]
    internal enum AESTransformMode
    {
        Encrypt = 0,
        Decrypt = 1
    }
    ...
    public class AESTransform : IAlgorithmTransform
    {
        private CipherMode m_cipherMode;
        private PaddingMode m_paddingValue;
        private AESTransformMode m_transformMode;
        ...
        internal AESTransform(byte[] rgbKey,
                            CipherMode mode,
                            byte[] rgbIV,
                            int blockSize,
                            int feedbackSize,
                            PaddingMode PaddingValue,
                            AESTransformMode transformMode)
        {
            if (rgbKey == null)
            ...
            int S1 = this.m_Nb > 6 ? 3 : 2;
            int S2 = this.m_Nb > 6 ? 4 : 3;

            // Precompute the modulus operations: these are performance killers when called frequently
            int[] encryptindex1 = new int[this.m_Nb];
            int[] encryptindex2 = new int[this.m_Nb];
            int[] encryptindex3 = new int[this.m_Nb];

            int[] decryptindex1 = new int[this.m_Nb];
            int[] decryptindex2 = new int[this.m_Nb];
            int[] decryptindex3 = new int[this.m_Nb];

            for (int j = 0; j < this.m_Nb; j++)
            {
                encryptindex1[j] = (j + 1) % this.m_Nb;
                encryptindex2[j] = (j + S1) % this.m_Nb;
                encryptindex3[j] = (j + S2) % this.m_Nb;
                decryptindex1[j] = (j - 1 + this.m_Nb) % this.m_Nb;
                decryptindex2[j] = (j - S1 + this.m_Nb) % this.m_Nb;
                decryptindex3[j] = (j - S2 + this.m_Nb) % this.m_Nb;
            }

            this.m_encryptindex = new int[this.m_Nb * 3];
            Array.Copy(encryptindex1, 0, this.m_encryptindex, 0, this.m_Nb);
            Array.Copy(encryptindex2, 0, this.m_encryptindex, this.m_Nb, this.m_Nb);
            Array.Copy(encryptindex3, 0, this.m_encryptindex, this.m_Nb * 2, this.m_Nb);

            this.m_decryptindex = new int[this.m_Nb * 3];
            Array.Copy(decryptindex1, 0, this.m_decryptindex, 0, this.m_Nb);
            Array.Copy(decryptindex2, 0, this.m_decryptindex, this.m_Nb, this.m_Nb);
            Array.Copy(decryptindex3, 0, this.m_decryptindex, this.m_Nb * 2, this.m_Nb);
            ...
            for (int i = 0; i < this.m_Nb; ++i)
            {
                ...
                int i2 = rgbIV[index++];
                int i3 = rgbIV[index++];
                this.m_IV[i] = i3 << 24 | i2 << 16 | i1 << 8 | i0;
            }
#if FEATURE_CRYPTO
            }
#endif // FEATURE_CRYPTO
            ...
            System.Buffer.BlockCopy(this.m_IV, 0, this.m_lastBlockBuffer, 0, this.m_blockSizeBytes);
#if FEATURE_CRYPTO
            }
#endif // FEATURE_CRYPTO
            ...
            if (this.m_cipherMode == CipherMode.CFB) {
                ...
                System.Buffer.BlockCopy(this.m_IV, 0, this.m_shiftRegister, 0, 4*this.m_Nb);
            }
#endif // FEATURE_CRYPTO

            this.Collector = new Collector(
                Enumerable.Range(0, this.m_Nr + 3).ToArray()
            );
        }
        ...
        public int TransformBlock(byte[] inputBuffer, int inputOffset, int inputCount, byte[] outputBuffer, int outputOffset)
        {
            ...
            if (this.m_transformMode == AESTransformMode.Encrypt)
            {
            ...
            else
            {
                ...
                if (this.m_paddingValue == PaddingMode.Zeros || this.m_paddingValue == PaddingMode.None) {
                    ...
                    return DecryptData(inputBuffer, inputOffset, inputCount, ref outputBuffer, outputOffset, this.m_paddingValue, false);
                }
                else {
#endif // FEATURE_CRYPTO
                ...
                if (this.m_depadBuffer == null)
                {
                    this.m_depadBuffer = new byte[InputBlockSize];
                    // copy the last InputBlockSize bytes to this.m_depadBuffer everything else gets processed and returned
                    int inputToProcess = inputCount - InputBlockSize;
                    System.Buffer.BlockCopy(inputBuffer, inputOffset + inputToProcess, this.m_depadBuffer, 0, InputBlockSize);
                ...
                else
                {
                    ...
                    int r = DecryptData(this.m_depadBuffer,
                                        0, this.m_depadBuffer.Length,
                                        ref outputBuffer,
                                        outputOffset,
                                        this.m_paddingValue,
                                        false);
        ...
        public byte[] TransformFinalBlock(byte[] inputBuffer, int inputOffset, int inputCount)
        {
            ...
            if (this.m_transformMode == AESTransformMode.Encrypt)
            {
                // If we're encrypting we can alway return what we compute because there's no this.m_depadBuffer
            ...
            else
            {
                ...
                {
                    byte[] temp = new byte[this.m_depadBuffer.Length + inputCount];
                    System.Buffer.BlockCopy(this.m_depadBuffer, 0, temp, 0, this.m_depadBuffer.Length);
                    System.Buffer.BlockCopy(inputBuffer, inputOffset, temp, this.m_depadBuffer.Length, inputCount);
                    byte[] transformedBytes = null;
                    DecryptData(temp,
                                0,
                                temp.Length,
                                       ...
                                       bool fLast)
        {
            ...
            int padSize = 0;
            int lonelyBytes = inputCount % this.m_inputBlockSize;
            ...
            if (fLast)
            {
                ...
                case PaddingMode.Zeros:
                    if (lonelyBytes != 0)
                        padSize = this.m_inputBlockSize - lonelyBytes;
                    break;
                ...
                fixed (int* encryptKeyExpansion = this.m_encryptKeyExpansion)
                {
                    fixed (int* T = s_T)
                    {
                        fixed (int* TF = s_TF)
                        {
                            ...
                            int iNumBlocks = (inputCount + padSize) / this.m_inputBlockSize;
                            int transformCount = outputOffset;
                            for (int blockNum = 0; blockNum < iNumBlocks; ++blockNum)
                            {

                                var block = this.Collector.NewBlock(
                                    inputBuffer.Skip(workBaseIndex).Take(this.m_blockSizeBytes).ToArray()
                                );

#if FEATURE_CRYPTO // see code:System.Security.Cryptography.RijndaelManaged#CoreCLRRijndaelModes
                                if (this.m_cipherMode == CipherMode.CFB) {
                                    Contract.Assert(this.m_blockSizeBytes <= this.m_Nb * sizeof(int), "this.m_blockSizeBytes <= this.m_Nb * sizeof(int)");
                                ...
                                {
                                    Contract.Assert(this.m_blockSizeBytes <= this.m_Nb * sizeof(int), "this.m_blockSizeBytes <= this.m_Nb * sizeof(int)");
                                    Buffer.Memcpy((byte*)work, 0, inputBuffer, workBaseIndex, this.m_blockSizeBytes);
                                }
                                else
                                {
                                    int padIndex = 0;
                                    ...
                                    for (int i = 0; i < this.m_Nb; ++i)
                                    {
                                        ...
                                        this.AddRound(block, ROUND_PAD, work);
                                }
#if FEATURE_CRYPTO
                                }
#endif // FEATURE_CRYPTO
                                ...
                                for (int i = 0; i < this.m_Nb; ++i)
                                {
                                    ...
                                    work[i] ^= this.m_lastBlockBuffer[i];
                                }
                                    this.AddRound(block, ROUND_CBC, work);
#if FEATURE_CRYPTO
                                }
#endif // FEATURE_CRYPTO

                                Enc(encryptindex, encryptKeyExpansion, T, TF, work, temp, block);
                                ...
                                if (this.m_cipherMode == CipherMode.CFB) {
                                    ...
                                    if (blockNum != iNumBlocks - 1 || padSize == 0) {
                                        for (int i = 0; i < this.m_Nb; ++i) {
                                            if (index >= workBaseIndex + this.m_inputBlockSize) break;
                                        ...
                                        byte[] tmpInputBuffer = new byte[this.m_inputBlockSize];
                                        System.Buffer.BlockCopy(inputBuffer, workBaseIndex, tmpInputBuffer, 0, lonelyBytes);
                                        System.Buffer.BlockCopy(padBytes, 0, tmpInputBuffer, lonelyBytes, padSize);
                                        index = 0;
                                        for (int i = 0; i < this.m_Nb; ++i) {
                                            if (index >= this.m_inputBlockSize) break;
                                    ...
                                    // shift this.m_lastBlockBuffer to the left by this.m_inputBlockSize bytes.
                                    index = 0;
                                    ...
                                    System.Buffer.BlockCopy(outputBuffer, blockNum * this.m_inputBlockSize, this.m_shiftRegister, this.m_blockSizeBytes - this.m_inputBlockSize, this.m_inputBlockSize);
                                } else {
#endif // FEATURE_CRYPTO
                                for (int i = 0; i < this.m_Nb; ++i)
                                {
                                    ...
                                    if (this.m_cipherMode == CipherMode.CBC) {
#endif // FEATURE_CRYPTO
                                fixed (int* pLastBlockBuffer = this.m_lastBlockBuffer)
                                {
                                    Contract.Assert(this.m_blockSizeBytes <= this.m_lastBlockBuffer.Length * sizeof(int), "this.m_blockSizeBytes <= this.m_lastBlockBuffer.Length * sizeof(int)");
                                    Buffer.Memcpy((byte*)pLastBlockBuffer, (byte*)temp, this.m_blockSizeBytes);
                                }
#if FEATURE_CRYPTO
                                    }
                                }
#endif // FEATURE_CRYPTO
                                workBaseIndex += this.m_inputBlockSize;
                            }
                        }
                    }
                }
                                ...
                                fixed (int* TF = s_TF)
                                {
                                    fixed (int* iT = s_iT)
                                    {
                                        fixed (int* iTF = s_iTF)
                                        {
                                            ...
                                            int iNumBlocks = inputCount / this.m_inputBlockSize;
                                            ...
                                            for (int blockNum = 0; blockNum < iNumBlocks; ++blockNum)
                                            {
                                                ...
                                                if (this.m_cipherMode == CipherMode.CFB) {
                                                    index = 0;
                                                    for (int i = 0; i < this.m_Nb; ++i) {
                                                        int i0 = this.m_shiftRegister[index++];
                                                        int i1 = this.m_shiftRegister[index++];
                                                        int i2 = this.m_shiftRegister[index++];
                                                        int i3 = this.m_shiftRegister[index++];
                                                        work[i] = i3 << 24 | i2 << 16 | i1 << 8 | i0;
                                                    }
                                                } else {
#endif // FEATURE_CRYPTO
                                                ...
                                                if (this.m_cipherMode == CipherMode.CFB) {
                                                    ...
                                                    System.Buffer.BlockCopy(inputBuffer, workBaseIndex, this.m_shiftRegister, this.m_blockSizeBytes - this.m_inputBlockSize, this.m_inputBlockSize);
                                                } else {
#endif // FEATURE_CRYPTO
                                                ...
                                                for (int i = 0; i < this.m_Nb; ++i)
                                                {
                                                    temp[i] ^= this.m_lastBlockBuffer[i];
                                                    // save the input buffer
                                                    int i0 = inputBuffer[index++];
                                                    int i1 = inputBuffer[index++];
                                                    int i2 = inputBuffer[index++];
                                                    int i3 = inputBuffer[index++];
                                                    this.m_lastBlockBuffer[i] = i3 << 24 | i2 << 16 | i1 << 8 | i0;
                                                }
#if FEATURE_CRYPTO
                                                    }
#endif // FEATURE_CRYPTO
                                            ...
                                            System.Buffer.BlockCopy(outputBuffer, 0, outputBuffer1, 0, outputBuffer.Length - padSize);
#if FEATURE_CRYPTO
                                                break;
                                            case PaddingMode.ANSIX923:
                                                if (inputCount == 0)
        ...
        unsafe void AddRound(Block block, int roundId, int * buf)
        {
            var result = new byte[this.m_Nb * 4];
            for (int i = 0; i < this.m_Nb; ++i)
            {
                result[i * 4] = (byte)(buf[i] & 0xFF);
                result[i * 4 + 1] = (byte)(buf[i] >> 8 & 0xFF);
                result[i * 4 + 2] = (byte)(buf[i] >> 16 & 0xFF);
                result[i * 4 + 3] = (byte)(buf[i] >> 24 & 0xFF);
            }
            block.AddRound(roundId, result);
        }
        ...
        private unsafe void Enc(int* encryptindex, int* encryptKeyExpansion, int* T, int* TF, int* work, int* temp, Block block = null)
        {
            for (int i = 0; i < this.m_Nb; ++i)
            {
            ...
            this.AddRound(block, ROUND_ENC, work);

            int* _encryptindex;
            int* _encryptKeyExpansion = &encryptKeyExpansion[this.m_Nb];
            for (int r = 1; r < this.m_Nr; ++r)
            {
                ...
                for (int i = 0; i < this.m_Nb; ++i)
                {
                    temp[i] = T[0 + (work[i] & 0xFF)] ^
                              ...
                              T[512 + ((work[_encryptindex[this.m_Nb]] >> 16) & 0xFF)] ^
                              T[768 + ((work[_encryptindex[this.m_Nb * 2]] >> 24) & 0xFF)] ^
                ...
                this.AddRound(block, ROUND_ENC + r, work);
            }
            ...
            for (int i = 0; i < this.m_Nb; ++i)
            {
                temp[i] = TF[0 + (work[i] & 0xFF)] ^
                          ...
                          TF[512 + ((work[_encryptindex[this.m_Nb]] >> 16) & 0xFF)] ^
                          TF[768 + ((work[_encryptindex[this.m_Nb * 2]] >> 24) & 0xFF)] ^
                          *_encryptKeyExpansion;
                _encryptindex++;
                _encryptKeyExpansion++;
            }

            this.AddRound(block, ROUND_ENC + this.m_Nr, temp);
        }
        ...
        {
            int keyIndex = this.m_Nb * this.m_Nr;
            for (int i = 0; i < this.m_Nb; ++i)
            {
            ...
            for (int r = 1; r < this.m_Nr; ++r)
            {
                ...
                for (int i = 0; i < this.m_Nb; ++i)
                {
                    temp[i] = iT[0 + ((work[i]) & 0xFF)] ^
                              ...
                              iT[512 + ((work[_decryptindex[this.m_Nb]] >> 16) & 0xFF)] ^
                              iT[768 + ((work[_decryptindex[this.m_Nb * 2]] >> 24) & 0xFF)] ^
            ...
            for (int i = 0; i < this.m_Nb; ++i)
            {
                temp[i] = iTF[0 + ((work[i]) & 0xFF)] ^
                          ...
                          iTF[512 + ((work[_decryptindex[this.m_Nb]] >> 16) & 0xFF)] ^
                          iTF[768 + ((work[_decryptindex[this.m_Nb * 2]] >> 24) & 0xFF)] ^
                          *_decryptKeyExpansion;
                _decryptindex++;
                _decryptKeyExpansion++;
            }
        ...
        private void GenerateKeyExpansion(byte[] rgbKey)
        {
            ...
            for (int i = 0; i < this.m_Nk; ++i)
            {
                ...
                int i2 = rgbKey[index++];
                int i3 = rgbKey[index++];
                this.m_encryptKeyExpansion[i] = i3 << 24 | i2 << 16 | i1 << 8 | i0;
            }

            if (this.m_Nk <= 6)
            {
                for (int i = this.m_Nk; i < this.m_Nb * (this.m_Nr + 1); ++i)
                {
                    iTemp = this.m_encryptKeyExpansion[i - 1];

                    if (i % this.m_Nk == 0)
                    {
                        iTemp = SubWord(rot3(iTemp));
                        iTemp = iTemp ^ s_Rcon[(i / this.m_Nk) - 1];
                    }

                    this.m_encryptKeyExpansion[i] = this.m_encryptKeyExpansion[i - this.m_Nk] ^ iTemp;
                }
            }
            else
            {
            ...
            for (int i = 0; i < this.m_Nb; ++i)
            {
                this.m_decryptKeyExpansion[i] = this.m_encryptKeyExpansion[i];
                this.m_decryptKeyExpansion[this.m_Nb * this.m_Nr + i] = this.m_encryptKeyExpansion[this.m_Nb * this.m_Nr + i];
            }

            for (int i = this.m_Nb; i < this.m_Nb * this.m_Nr; ++i)
            {
                int key = this.m_encryptKeyExpansion[i];
                int mul02 = MulX(key);
                int mul04 = MulX(mul02);
                int mul08 = MulX(mul04);
                int mul09 = key ^ mul08;
                this.m_decryptKeyExpansion[i] = mul02 ^ mul04 ^ mul08 ^ rot3(mul02 ^ mul09) ^ rot2(mul04 ^ mul09) ^ rot1(mul09);
            }
        }
        ...
        static int ROUND_PAD = 0;
        static int ROUND_CBC = 1;
        static int ROUND_ENC = 2;
    }
}