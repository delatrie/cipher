...
Для работы с генератором требуется оболочка `powershell`. Загрузить последнюю версию можно [по этой ссылке][1].

После установки `powershell` загрузите последний релиз модуля генератора [по этой ссылке][2]. Распакуйте содержимое архива в директорию по вашему выбору.
...
1. Создает подготавливает алгоритм шифрования AES со следующими параметрами:
...
1. Применяет алгоритм защифрования к открытому тексту. Размещает в директории следующие файлы:
